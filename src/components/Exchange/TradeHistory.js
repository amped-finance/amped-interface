...
import { useTrades, useLiquidationsData, useAllTradesHistory, useRawData } from "domain/legacy";
...
export default function TradeHistory(props) {
  ...
  const createHistory = useAllTradesHistory(chainId, account); 
  const liquidationsData = useLiquidationsData(chainId, account);
  const excutedHistory = useRawData(chainId, account)
  
  const tradeHistoryData = useMemo(() => {
    if((!createHistory || createHistory.length === 0) && (!excutedHistory || excutedHistory.length === 0)) {
      return []
    } else {
      let totalArray = createHistory.concat(excutedHistory)
      totalArray.sort((a, b) => b.timestamp - a.timestamp);
      return totalArray
    }
  }, [createHistory, excutedHistory])
  ...
  const tradesWithMessages = useMemo(() => {
    ...
    const historyData = tradeHistoryData
      .map((trade) => ({
        msg: getMsg(trade),
        ...trade,
      }))
      ...
      // return []
    const _historyData = historyData.sort((prev, next) => parseFloat(next.data.timestamp) - parseFloat(prev.data.timestamp))
    return _historyData
